"""
Code sourced from: https://matplotlib.org/stable/gallery/animation/double_pendulum.html

air resistance and bearing friction damping coefficients added to increase realism
this code will be used in conjunction with a lagrangian code externally sourced to compare
...
L1 = 0.245  # length of pendulum 1 in m
L2 = 0.2 # length of pendulum 2 in m
L = L1 + L2  # maximal length of the combined pendulum
M1 = 0.3553  # mass of pendulum 1 in kg
M2 = 0.2379 # mass of pendulum 2 in kg
...
def derivs(t, state):
    ...
    dydx[1] = ((M2 * L1 * state[1] * state[1] * sin(delta) * cos(delta)
                ...
                - (M1+M2) * G * sin(state[0]))
               / den1)
    dydx[1] -= b1 * state[1] #bearing friction on pendulum 1
    dydx[1] -= c1 * state[1] * abs(state[1]) #air resistance on pendulum 1
    dydx[2] = state[3]
